...
const checkRegistrationOpen = async () => {
    const { data } = await supabase
        ...
        .select('value')
        .eq('key', 'course_registration')
        .single();
    
    // Default to true if not set, or check strictly for 'true'
...
const applyForCourse = async (req, res) => {
  ...
  try {
    ...
    if (!isOpen) {
        return res.status(403).json({ error: "Course registration is currently CLOSED." });
    }
            ...
            if (record.courses.acad_session === course.acad_session) {
                if (record.courses.slot === course.slot) {
                    return res.status(400).json({ 
                        error: `You have already enrolled in a course on slot ${course.slot}. Please drop that course before applying.` 
                    });
                }
            }
        }
...
const dropCourse = async (req, res) => {
  ...
  try {
    ...
    const isOpen = await checkRegistrationOpen();
    if (!isOpen) {
        return res.status(403).json({ error: "Course dropping is currently CLOSED." });
    }
...
const getStudentRecords = async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('enrollments')
      .select(`enrollment_id, status, grade, course_id, courses(course_id, course_code, title, acad_session)`)
      .eq('student_id', student_id)
...
const submitInstructorFeedback = async (req, res) => {
    ...
    try {
        ...
        if(!enr || enr.status !== "ENROLLED" || String(enr.course.faculty_id) !== String(instructor_id)) return res.status(403).json({ error: "Invalid feedback submission." });
